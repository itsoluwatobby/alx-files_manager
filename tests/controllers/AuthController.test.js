...
describe('AuthController TestCase', () => {
  const mockUser = {
  ...
  describe('GET: /connect', () => {
    ...
    it('Success for existing user', (done) => {
      this.timeout(5000);
      request.get('/connect')
  ...
  describe('GET: /disconnect', () => {
    ...
    it('Fails for invalid user', (done) => {
      this.timeout(5000);
      request.get('/disconnect')
        .set('X-Token', 'raboof')
        .expect(401)
    ...
    it('Success with valid "X-Token" field', (done) => {
      request.get('/disconnect')
        .set('X-Token', token)
        .expect(204)
        .end((err, res) => {
...
})t('Set and get expired value', async () => {
	    await redisClient.set('v_key', 20, 2);
	    setTimeout(async () => {
		          expect(await redisClient.get('test_key')).to.not.equal('20');
		        }, 3000);
	  });
  ...
  it('Set and get deleted value', async function () {
	      ...
	      setTimeout(async () => {
		            console.log('del: v_key ->', await redisClient.get('v_key'));
